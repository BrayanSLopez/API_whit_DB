...
def create_user():
    ...
    if not username or not password or not email:
        return jsonify({'error': 'El nombre de usuario, la contraseña y el email son obligatorios'}), 400
    user = service.create_user(username, password, email, full_name)
    return jsonify({'id': user.id, 'username': user.username, 'email': user.email, 'full_name': user.full_name}), 201
...
def update_user(user_id):
    ...
    if user:
        return jsonify({'id': user.id, 'username': user.username, 'email': user.email, 'full_name': user.full_name}), 200
    return jsonify({'error': 'Usuario no encontrado'}), 404
